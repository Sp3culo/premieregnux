...
init = Init(
    disk_c=os.getenv('AEGNUX_DISK_C_DIR', f'{home}/.wine/drive_c/Program Files'),
    current_dir=os.getenv('AEGNUX_APP_DIR', f'{home}/.wine/drive_c/Program Files/Adobe'),
    product=[p.strip() for p in os.getenv('AEGNUX_PRODUCT', 'Adobe Premiere Pro').split(',')],
    version=[v.strip() for v in os.getenv('AEGNUX_VERSION', '2022,2023,2024,2025').split(',')]
)
...
def get_aegnux_installation_dir():
    aegnux_dir = Path(init.get_adobe_path())
...
def get_ae_install_dir():
    ae_dir = get_aegnux_installation_dir()
    return ae_dir
...
def get_product_install_dir(product_name: str, version: str = None) -> Path:
    info = init.get_product_info(product_name, version)
    if info is None:
        raise ValueError(f"Produit inconnu : {product_name}")
    base = Path(init.get_disk_c_path())
    return base / info['install_dir']

def get_product_exe(product_name: str, version: str = None) -> Path | None:
    install_dir = get_product_install_dir(product_name, version)
...
def is_product_installed(product_name: str, version: str = None) -> bool:
    exe = get_product_exe(product_name, version)
    return exe is not None and exe.exists()

def run_product(product_name: str, version: str = None):
    exe_path = get_product_exe(product_name, version)
    if exe_path is None or not exe_path.exists():
        raise FileNotFoundError(f"{product_name} n'est pas installé.")
    env = os.environ.copy()
    env['WINEPREFIX'] = str(get_wineprefix_dir())
    env['PATH'] = get_wine_bin_path_env(env.get('PATH'))
    subprocess.Popen([str(get_wine_bin()), str(exe_path)], env=env)